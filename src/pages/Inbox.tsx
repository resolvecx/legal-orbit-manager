...
import { Search, Filter, Mail, MessageSquare, MessageCircle, Phone, MoreHorizontal, Paperclip, Send, ArrowLeft, Check, CheckCheck } from "lucide-react";
...
const Inbox = () => {
  ...
  const getChannelIcon = (channel: string) => {
    switch (channel) {
      case "Email":
        return <div className="flex items-center gap-1 px-2 py-1 bg-blue-100 text-blue-700 rounded-full text-xs">
          <span>📧</span>
          <span>Email</span>
        </div>;
      case "WhatsApp":
        return <div className="flex items-center gap-1 px-2 py-1 bg-green-100 text-green-700 rounded-full text-xs">
          <span>💬</span>
          <span>WhatsApp</span>
        </div>;
      case "Live Chat":
        return <div className="flex items-center gap-1 px-2 py-1 bg-purple-100 text-purple-700 rounded-full text-xs">
          <span>🔵</span>
          <span>Live Chat</span>
        </div>;
      ...
      default:
        return <div className="flex items-center gap-1 px-2 py-1 bg-gray-100 text-gray-700 rounded-full text-xs">
          <span>📧</span>
          <span>Email</span>
        </div>;
    }
  ...
  return <div className="h-screen flex bg-background">
      ...
      <div className="w-80 border-r border-border bg-card flex flex-col">
        <div className="p-4 border-b border-border">
          <div className="flex items-center justify-between mb-4">
            <ArrowLeft className="w-5 h-5 text-muted-foreground cursor-pointer hover:text-foreground" onClick={() => navigate('/')} />
        ...
        <div className="flex-1 overflow-y-auto">
          {mockConversations.map(conversation => <div key={conversation.id} className={`p-4 border-b border-border cursor-pointer hover:bg-accent/50 ${selectedConversation.id === conversation.id ? 'bg-accent' : ''}`} onClick={() => setSelectedConversation(conversation)}>
      ...
      <div className="flex-1 flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
          {mockMessages.map(message => <div key={message.id} className={`flex ${message.sender === 'agent' ? 'justify-end' : 'justify-start'}`}>
              <div className={`max-w-[70%] ${message.sender === 'agent' ? 'bg-primary text-primary-foreground' : 'bg-muted'} rounded-lg p-3`}>
                <p className="text-sm">{message.content}</p>
                <div className="flex items-center justify-between mt-2 text-xs opacity-70">
                  <span>{message.timestamp}</span>
                  {message.sender === 'agent' && message.status && <div className="flex items-center gap-1">
        ...
        <div className="p-4">
          ...
          <Card className="mb-4">
            <CardContent className="p-4">
              ...
              <div className="space-y-3">
                {mockRecentActivity.map(activity => <div key={activity.id} className="flex items-start gap-3">
          ...
          <Card>
            <CardContent className="p-4">
              ...
              <div className="space-y-3">
                {mockPreviousCases.map(case_item => <div key={case_item.id} className="border border-border rounded-lg p-3">
                    <div className="flex items-center justify-between mb-1">
